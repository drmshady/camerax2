package com.example.camerax

import android.hardware.camera2.CameraCaptureSession
import android.hardware.camera2.CaptureRequest
import android.hardware.camera2.CaptureResult
import android.hardware.camera2.TotalCaptureResult
import java.util.concurrent.atomic.AtomicReference

data class CaptureSnapshot(
    val iso: Int?,
    val shutterNs: Long?,
    val focusDistanceDiopters: Float?,
    val afState: Int?,
    val aeState: Int?,
    val awbState: Int?
)

/**
 * Stores latest Camera2 values observed via Camera2Interop session capture callbacks.
 * This is intentionally lightweight and thread-safe.
 */
class CaptureResultStore {

    private val isoRef = AtomicReference<Int?>(null)
    private val shutterNsRef = AtomicReference<Long?>(null)
    private val focusDistRef = AtomicReference<Float?>(null)

    private val afStateRef = AtomicReference<Int?>(null)
    private val aeStateRef = AtomicReference<Int?>(null)
    private val awbStateRef = AtomicReference<Int?>(null)

    val sessionCallback = object : CameraCaptureSession.CaptureCallback() {
        override fun onCaptureCompleted(
            session: CameraCaptureSession,
            request: CaptureRequest,
            result: TotalCaptureResult
        ) {
            val iso = result.get(CaptureResult.SENSOR_SENSITIVITY)
            val shutterNs = result.get(CaptureResult.SENSOR_EXPOSURE_TIME)
            val focusDist = result.get(CaptureResult.LENS_FOCUS_DISTANCE) // diopters

            val afState = result.get(CaptureResult.CONTROL_AF_STATE)
            val aeState = result.get(CaptureResult.CONTROL_AE_STATE)
            val awbState = result.get(CaptureResult.CONTROL_AWB_STATE)

            if (iso != null) isoRef.set(iso)
            if (shutterNs != null) shutterNsRef.set(shutterNs)
            if (focusDist != null) focusDistRef.set(focusDist)

            if (afState != null) afStateRef.set(afState)
            if (aeState != null) aeStateRef.set(aeState)
            if (awbState != null) awbStateRef.set(awbState)
        }
    }

    // For CameraController Step 4
    fun latestIso(): Int? = isoRef.get()
    fun latestShutterNs(): Long? = shutterNsRef.get()
    fun latestFocusDistance(): Float? = focusDistRef.get()

    fun latestAfState(): Int? = afStateRef.get()
    fun latestAeState(): Int? = aeStateRef.get()
    fun latestAwbState(): Int? = awbStateRef.get()

    // Deterministic snapshot without re-query
    fun snapshot(): CaptureSnapshot =
        CaptureSnapshot(
            iso = isoRef.get(),
            shutterNs = shutterNsRef.get(),
            focusDistanceDiopters = focusDistRef.get(),
            afState = afStateRef.get(),
            aeState = aeStateRef.get(),
            awbState = awbStateRef.get()
        )
}
