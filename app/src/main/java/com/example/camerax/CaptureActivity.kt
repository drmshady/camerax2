        ...
        uploadJob = lifecycleScope.launch {
            try {
                ...
                if (result.success) {
                    ...
                    android.widget.Toast.makeText(this@CaptureActivity, "Sent: " + zipFile.name, android.widget.Toast.LENGTH_SHORT).show()
                    binding.sendStatusText.text = "Done"
                } else {
                    android.widget.Toast.makeText(this@CaptureActivity, "Send failed: " + (result.errorMessage ?: result.httpCode.toString()), android.widget.Toast.LENGTH_LONG).show()
                    binding.sendStatusText.text = "Failed"
                }
            } catch (t: Throwable) {
                android.util.Log.e(TAG, "startUploadExistingZip crashed: " + t.message, t)
                android.widget.Toast.makeText(this@CaptureActivity, "Send error: " + (t.message ?: "unknown"), android.widget.Toast.LENGTH_LONG).show()
